...
> [!NOTE]
> This is a fork of [hemanth/functional-programming-jargon](https://github.com/hemanth/functional-programming-jargon)

> [!WARNING]
> This is a work in progress. Some examples are in Kotlin, but most are in JavaScript. I am working on translating all examples to Kotlin.

Functional programming (FP) provides many advantages, and its popularity has been increasing as a result. However, each programming paradigm comes with its own unique jargon and FP is no exception. By providing a glossary, we hope to make learning FP easier.

Examples are presented in Kotlin. [Take Kotlin Tour](https://kotlinlang.org/docs/kotlin-tour-welcome.html)
...
filter(isA(Int::class), listOf(0, "1", 2)) // [0, 2]
```
...
> [!NOTE]
> Kotlin does not support partial application out of the box. So, we will use a Arrow library to achieve this.
...
val fivePlus = add3.partially1(2).partially2(3) // (c) => 2 + 3 + c
...
> [!NOTE]
> Arrow provides a `curried` function that can be used to curry a function.
...
infix fun <A, B, C> ((B) -> C).compose(g: (A) -> B): (A) -> C = { x: A -> this(g(x)) } // Definition
val floorAndToString = { x: Double -> x.toString() }.compose { x: Double -> floor(x) } // Usage
...
class Max(private val a: Int) {
    ...
    override fun toString() = "Max($this.a)"
}
```

```kotlin
Max(2).compose(Max(3)).compose(Max(5)) // => Max(5)
```
...
data class Person(val name: String, val age: Int) // Definition
```
...
```kotlin
john.age + FIVE == Person("John", 30).age + 5 // true
```
...
> [!NOTE]
> Kotlin not provide a Option type out of the box. Arrow provides a `Option` type that similar to the one in Scala.

```kotlin
import arrow.core.Some

Some(1).map { it } // = Option.Some(1)
```
...
Kotlin has `*Of` functions to make collections a pointed functor.
